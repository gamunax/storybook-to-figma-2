...
import { _DialogContainerBase } from './dialog-container';
 ...
 export const enum DialogState {OPEN, CLOSING, CLOSED}
 ...
 export class DialogRef<T, R = any> {
  ...
  private readonly _destroyed$ = new Subject<void>();
   ...
   constructor(
     ...
     readonly id: string = `atlas-dialog-${uniqueId++}`) {
    this.disableClose = _containerInstance?._config.disableClose;
     ...
     _overlayRef.detachments().pipe(takeUntil(this._destroyed$)).subscribe(() => {
     ...
     _overlayRef.keydownEvents()
       .pipe(
         takeUntil(this._destroyed$), filter(event => {
     ...
     _overlayRef.backdropClick().pipe(
 ...
 // Move this back into `DialogRef` when we provide an official mock dialog ref.
